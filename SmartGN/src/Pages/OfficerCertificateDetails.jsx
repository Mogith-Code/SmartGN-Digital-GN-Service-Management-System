...
function OfficerCertificateDetails({ onOpenHelp }) {
  ...
  const loadCertDetails = async () => {
    try {
      ...
      if (found) {
        const formatted = {
          ...
          submittedDate: found.request_date
            ? found.request_date.split("T")[0]
            : found.submittedDate || "",
          ...
          verifiedAnnualIncome:
            found.verifiedAnnualIncome || found.annualIncome || "",
        };
        setCertRequest(formatted);

        // Seed official state
        const isIncome =
          formatted.type === "Income Certificate" ||
        ...
        setCertificateNo(
          formatted.certificateNo ||
            (isIncome
              ? `IC/2026/${Math.floor(1000 + Math.random() * 9000)}`
              : `CC/2026/${Math.floor(1000 + Math.random() * 9000)}`),
        );
    ...
    if (saved) {
      ...
      if (found) {
        ...
        const isIncome =
          found.type === "Income Certificate" ||
          found.certificate_type === "INCOME" ||
          found.certificateType === "INCOME";
        setPersonalKnown(found.personalKnown || "No");
        setPersonalKnownSince(found.personalKnownSince || "");
        setNatureOfOtherEvidences(
          found.natureOfOtherEvidences || "Utility Bill",
        );
  ...
  const handleApprove = async () => {
    if (!signatureMatch || !billsVerified) {
      const confirmApprove = window.confirm(
        "You have not checked all Officer Quick Check items. Do you still want to approve this application?",
      );
    ...
    try {
      ...
      setDocumentAuditCheck(true);
      alert(
        `Certificate request ${id} has been Approved and Issued successfully!`,
      );
      ...
      if (saved) {
        ...
        const updated = list.map((c) =>
          c.id === id || c.request_id === id
            ? { ...c, ...payload, status: "APPROVED" }
            : c,
        );
      ...
      if (residentSaved) {
        ...
        const updatedRes = resList.map((c) =>
          c.id === id || c.request_id === id
            ? { ...c, ...payload, status: "APPROVED" }
            : c,
        );
      ...
      setDocumentAuditCheck(true);
      alert(
        `Certificate request ${id} has been Approved and Issued successfully! (offline data synced)`,
      );
      navigate("/dashboard/officer/certificates", {
        state: {
          successUser: `${profile.firstName} ${profile.lastName}`,
  ...
  const handleReject = async () => {
    ...
    try {
      ...
      setCertRequest((prev) => (prev ? { ...prev, status: "Rejected" } : null));
      alert(
        `Certificate request ${id} has been Rejected. (offline data synced)`,
      );
      navigate("/dashboard/officer/certificates", {
        state: {
          successUser: `${profile.firstName} ${profile.lastName}`,
  ...
  const isCharacterCert =
    certRequest.type === "Character Certificate" ||
    certRequest.certificate_type === "CHARACTER";
        ...
        <main className="flex-1 p-10 bg-[#F7FAFC] overflow-y-auto">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start mb-8">
            ...
            <div className="lg:col-span-2 bg-white border border-[#cbd5e1] rounded-2xl p-8 shadow-sm text-left">
              <div className="flex items-center gap-3.5 border-b border-[#f1f5f9] pb-4 mb-5">
                ...
                <h3 className="text-[17px] font-bold text-[#1B365D] m-0">
                  Applicant Information Form Details
                </h3>
              </div>
              ...
              {!isCharacterCert ? (
                ...
                <div className="flex flex-col gap-4">
                  ...
                  <div>
                    ...
                    <span className="text-[14.5px] font-bold text-[#1e293b]">
                      {certRequest.address || "45/2, Temple Road, Maharagama."}
                    </span>
                  </div>
                  ...
                  {certRequest.incomeStream === "Paddy" && (
                    <div className="mt-4 p-4 bg-slate-50 border border-slate-200 rounded-xl">
                      <h4 className="text-[14px] font-bold text-[#1B365D] uppercase tracking-wider mb-3">
                        Paddy / Agriculture Stream Details
                      </h4>
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-[13.5px]">
                        ...
                        <div>
                          ...
                          <span className="text-slate-800 font-bold">
                            {certRequest.landAmount || "N/A"}
                          </span>
                        </div>
                        <div>
                          <span className="text-slate-500 font-semibold block">
                            Applicant Ownership Identity:
                          </span>
                          <span className="text-slate-800 font-bold">
                            {certRequest.ownerIdentity || "N/A"}
                          </span>
                        </div>
                        <div>
                          <span className="text-slate-500 font-semibold block">
                            Amount of Paddy Obtained:
                          </span>
                          <span className="text-slate-800 font-bold">
                            {certRequest.amountObtained || "N/A"}
                          </span>
                        </div>
                        <div>
                          <span className="text-slate-500 font-semibold block">
                        ...
                        <div>
                          ...
                          <span className="text-slate-800 font-bold text-red-600">
                            Rs. {certRequest.expenses || "0"}
                          </span>
                        </div>
                        <div>
                          <span className="text-slate-500 font-semibold block">
                  ...
                  {certRequest.incomeStream === "Business" && (
                    <div className="mt-4 p-4 bg-slate-50 border border-slate-200 rounded-xl">
                      <h4 className="text-[14px] font-bold text-[#1B365D] uppercase tracking-wider mb-3">
                        Business / Commercial Stream Details
                      </h4>
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-[13.5px]">
                        <div>
                          <span className="text-slate-500 font-semibold block">
                        ...
                        <div>
                          ...
                          <span className="text-slate-800 font-bold">
                            {certRequest.businessNature || "N/A"}
                          </span>
                        </div>
                        <div>
                          <span className="text-slate-500 font-semibold block">
                          ...
                          <span className="text-slate-800 font-bold">
                            {certRequest.taxReceiptNumber || "N/A"}
                          </span>
                        </div>
                        <div>
                          <span className="text-slate-500 font-semibold block">
                          ...
                          <span className="text-slate-800 font-bold">
                            Rs. {certRequest.dailyMonthlyIncome || "0"}
                          </span>
                        </div>
                        <div>
                          <span className="text-slate-500 font-semibold block">
                  ...
                  {certRequest.incomeStream === "Laborer" && (
                    <div className="mt-4 p-4 bg-slate-50 border border-slate-200 rounded-xl">
                      <h4 className="text-[14px] font-bold text-[#1B365D] uppercase tracking-wider mb-3">
                        Carpenter / Masonry / Hired Laborer Stream Details
                      </h4>
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-[13.5px]">
                        <div>
                          <span className="text-slate-500 font-semibold block">
                        ...
                        <div>
                          ...
                          <span className="text-slate-800 font-bold">
                            Rs. {certRequest.monthlyIncome || "0"}
                          </span>
                        </div>
                      </div>
                    </div>
                ...
                <div className="flex flex-col gap-6">
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-[13.5px] pl-2">
                      <div>
                        <span className="text-slate-500 font-semibold block">
                          (a) District & Divisional Secretariat:
                        </span>
                        <span className="text-slate-800 font-bold">
                          {certRequest.divisionalSecretariat ||
                            "(Not specified)"}
                        </span>
                      </div>
                      <div>
                        <span className="text-slate-500 font-semibold block">
                          (b) Grama Niladhari Division & Number:
                        </span>
                        <span className="text-slate-800 font-bold">
                          {certRequest.gnDivisionNumber || "(Not specified)"}
                        </span>
                      </div>
                      <div>
                        <span className="text-slate-500 font-semibold block">
                          (c) Personally known to Grama Niladhari?
                        </span>
                        <span className="text-slate-800 font-bold">
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-[13.5px] pl-2">
                      ...
                      <div>
                        ...
                        <span className="text-slate-800 font-bold">
                          {certRequest.age || "(Not specified)"} Years
                        </span>
                      </div>
                      <div>
                        <span className="text-slate-500 font-semibold block">
                        ...
                        <span className="text-slate-800 font-bold">
                          {certRequest.civilStatus || "(Not specified)"}
                        </span>
                      </div>
                      <div>
                        <span className="text-slate-500 font-semibold block">
                          (f) Sri Lankan Nationality:
                        </span>
                        <span className="text-slate-800 font-bold">
                      ...
                      <div>
                        ...
                        <span className="text-slate-800 font-bold">
                          {certRequest.religion || "(Not specified)"}
                        </span>
                      </div>
                      <div>
                        <span className="text-slate-500 font-semibold block">
                        ...
                        <span className="text-slate-800 font-bold">
                          {certRequest.occupation || "Student / Unemployed"}
                        </span>
                      </div>
                      <div>
                        <span className="text-slate-500 font-semibold block">
                          (i) Period of Residence in Village:
                        </span>
                        <span className="text-slate-800 font-bold">
                          {certRequest.villagePeriod || "(Not specified)"}
                        </span>
                      </div>
                      <div>
                        <span className="text-slate-500 font-semibold block">
                        ...
                        <span className="text-slate-800 font-bold font-mono text-[14px]">
                          {certRequest.nic}
                        </span>
                      </div>
                      <div className="md:col-span-2">
                        ...
                        <span className="text-slate-800 font-bold">
                          {certRequest.electoralRegister || "Registered"}
                        </span>
                      </div>
                      <div>
                        <span className="text-slate-500 font-semibold block">
                        ...
                        <span className="text-slate-800 font-bold">
                          {certRequest.fatherName || "(Not specified)"}
                        </span>
                      </div>
                      <div>
                        <span className="text-slate-500 font-semibold block">
                        ...
                        <span className="text-slate-800 font-bold">
                          {certRequest.fatherAddress || "(Not specified)"}
                        </span>
                      </div>
                      <div className="md:col-span-2">
                        <span className="text-slate-500 font-semibold block">
                          (n) Purpose of Certificate:
                        </span>
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-[13.5px] pl-2">
                      <div>
                        ...
                        <span className="text-slate-800 font-bold">
                          {certRequest.gnPeriod || "(Not specified)"}
                        </span>
                      </div>
                      <div>
                        <span className="text-slate-500 font-semibold block">
                          (b) Nature of residence proof:
                        </span>
                        <span className="text-slate-800 font-bold">
                          {certRequest.natureOfOtherEvidences ||
                            "Utility bills, GN registry"}
                        </span>
                      </div>
                      <div>
                        <span className="text-slate-500 font-semibold block">
                          (c) Convicted by Court of Law?
                        </span>
                        <span
                      ...
                      <div>
                        <span className="text-slate-500 font-semibold block">
                          (d) Interested in public/social activities?
                        </span>
                        <span className="text-slate-800 font-bold">
            ...
            <div className="flex flex-col gap-6 text-left">
              ...
              <div className="bg-white border border-[#cbd5e1] rounded-2xl p-6 shadow-sm">
                ...
                <div className="flex flex-col gap-4">
                  ...
                  <div className="flex gap-3 items-start">
                    <div
                      className={`flex items-center justify-center w-5 h-5 rounded-full text-white text-[11px] font-bold mt-0.5 flex-shrink-0 ${
              ...
              <div className="bg-white border border-[#cbd5e1] rounded-2xl p-6 shadow-sm">
                ...
                <div className="flex flex-col gap-3">
                  <div className="flex justify-between items-center bg-[#f8fafc] border border-gray-100 p-3.5 rounded-xl">
                    ...
                    <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-[10.5px] font-bold bg-green-100 text-green-700 uppercase">
                      Issued
                    </span>
                  </div>
                </div>
              ...
              <div className="bg-white border border-[#fedc9b] rounded-2xl p-8 shadow-md text-left mb-8 animate-in fade-in slide-in-from-bottom duration-200">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-[13.5px] font-sans">
                  ...
                  <div className="flex flex-col">
                    <label
                      htmlFor="assessKnown"
                      className="font-bold text-[#334155] mb-1.5"
                    >
                  ...
                  <div className="flex flex-col">
                    <label
                      ...
                      className="font-bold text-[#334155] mb-1.5"
                    >
                      If yes, since when? (e.g. 3 years, Birth) :
                    </label>
                    <input
                      type="text"
                      id="assessSince"
                  ...
                  <div className="flex flex-col">
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setNatureOfOtherEvidences(e.target.value)
                      }
                    />
                  </div>
                  ...
                  <div className="flex flex-col">
                    <label
                      htmlFor="assessPublic"
                      className="font-bold text-[#334155] mb-1.5"
                    >
                      Applicant interest in social work/community? :
                    </label>
                    <select
                      id="assessPublic"
                      className="w-full py-2 px-3 border border-[#cbd5e1] rounded-lg bg-white"
                      ...
                      onChange={(e) =>
                        setPublicActivitiesInterest(e.target.value)
                      }
                    >
                      <option value="No">No</option>
                      <option value="Yes">Yes</option>
                  ...
                  <div className="flex flex-col">
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setPublicActivitiesDetails(e.target.value)
                      }
                  ...
                  <div className="flex flex-col">
                    <label
                      htmlFor="assessSerial"
                      className="font-bold text-[#334155] mb-1.5"
                    >
                      Generated Certificate Serial Number :
                    </label>
                    <input
                      type="text"
            ...
            (certRequest.status === "Pending" ||
              ...
              <div className="bg-white border border-[#fedc9b] rounded-2xl p-8 shadow-md text-left mb-8 animate-in fade-in slide-in-from-bottom duration-200">
                <div className="flex items-center gap-2 border-b border-[#fedc9b]/40 pb-4 mb-6 font-sans">
                  <span className="text-xl">✍️</span>
                  <h3 className="text-[17px] font-bold text-[#854d0e] m-0">
                    Grama Niladhari Official Income Verification Form
                  </h3>
                </div>
          ...
          <div className="bg-white border border-[#cbd5e1] rounded-2xl p-8 shadow-sm text-left mb-8">
            <div className="flex justify-between items-center border-b border-[#f1f5f9] pb-4 mb-6">
              ...
              <button
                onClick={() =>
                  alert("Downloading all supportive files securely...")
                }
              ...
              <div className="lg:col-span-2 flex gap-5 flex-wrap">
                ...
                <div className="w-[200px] border border-[#cbd5e1] rounded-xl overflow-hidden bg-[#f8fafc] shadow-sm">
                  <div className="h-[110px] bg-slate-200 flex items-center justify-center text-[#94a3b8] text-[28px] border-b border-[#cbd5e1] relative">
                    📄
                    <span className="absolute left-2 top-2 text-[10px] font-bold bg-black/60 text-white px-1.5 py-0.5 rounded">
                      PDF • 1.2MB
                    </span>
                  </div>
                  <div className="p-3 text-[13px]">
                    <span className="block font-bold text-[#1e293b] truncate">
                      Proof of Residence
                    </span>
