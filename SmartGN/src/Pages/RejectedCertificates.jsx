...
function RejectedCertificates({ onOpenHelp }) {
  ...
  const loadRejected = async () => {
    try {
      ...
      const rejected = data.filter(c => c.status === 'REJECTED' || c.status === 'Rejected')
      setRejectedList(rejected.map((c, index) => ({
        id: c.id || c.request_id,
        type: c.type || (c.certificate_type === 'INCOME' ? 'Income Certificate' : 'Character Certificate'),
        requestedDate: c.submittedDate || (c.request_date ? c.request_date.split('T')[0] : ''),
        approvedDate: c.approvedDate || '',
        purpose: c.purpose,
        reason: c.rejectionReason || "Uploaded Full Name doesn't match with the name in NIC",
        isActive: index === 0
      })))
    } catch (err) {
      console.warn('API connection offline. Loading rejected certificates from local fallback.')
      const localData = localStorage.getItem('smartgn_certificates')
      if (localData) {
        const allCerts = JSON.parse(localData)
        const rejected = allCerts.filter(c => c.status === 'REJECTED' || c.status === 'Rejected')
        setRejectedList(rejected.map((c, index) => ({
          id: c.id || c.request_id,
          type: c.type || (c.certificate_type === 'INCOME' ? 'Income Certificate' : 'Character Certificate'),
          requestedDate: c.submittedDate || (c.request_date ? c.request_date.split('T')[0] : ''),
        ...
        localStorage.setItem('smartgn_certificates', JSON.stringify(defaultRejected.map(p => ({
          ...p,
          status: 'REJECTED',
          certificate_type: p.type === 'Income Certificate' ? 'INCOME' : 'CHARACTER',
    ...
    <div className="flex flex-col min-h-screen w-full bg-[#F7FAFC]">
      ...
      <div className="flex flex-1 w-full">
        ...
        <main className="flex-1 p-10 bg-[#F7FAFC] overflow-y-auto relative">
          ...
          {loading ? (
            <div className="text-center py-20 text-[#64748b] text-[15px] font-medium">Loading rejected requests...</div>
