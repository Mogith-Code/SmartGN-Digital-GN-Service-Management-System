...
function PendingCertificates({ onOpenHelp }) {
  ...
  const loadPending = async () => {
    try {
      ...
      const pending = data.filter(c => c.status === 'PENDING' || c.status === 'Pending')
      setPendingList(pending.map((c, index) => ({
        ...
        purpose: c.purpose,
        stageKey: index % 3 === 0 ? 'verification' : (index % 3 === 1 ? 'signature' : 'audit'),
        isActive: index === 0
      })))
    } catch (err) {
      console.warn('API connection offline. Loading pending certificates from local fallback.')
      const localData = localStorage.getItem('smartgn_certificates')
      if (localData) {
        const allCerts = JSON.parse(localData)
        const pending = allCerts.filter(c => c.status === 'PENDING' || c.status === 'Pending')
        setPendingList(pending.map((c, index) => ({
          id: c.id || c.request_id,
          type: c.type || (c.certificate_type === 'INCOME' ? 'Income Certificate' : 'Character Certificate'),
          requestedDate: c.submittedDate || (c.request_date ? c.request_date.split('T')[0] : ''),
        ...
        localStorage.setItem('smartgn_certificates', JSON.stringify(defaultPending.map(p => ({
          ...p,
          status: 'PENDING',
    ...
    <div className="flex flex-col min-h-screen w-full bg-[#F7FAFC]">
      ...
      <div className="flex flex-1 w-full">
        ...
        <main className="flex-1 p-10 bg-[#F7FAFC] overflow-y-auto relative">
          ...
          {loading ? (
            <div className="text-center py-20 text-[#64748b] text-[15px] font-medium">Loading pending requests...</div>
